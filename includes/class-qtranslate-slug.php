...
class QtranslateSlug {
	...
	*/
	...
	function notice_dependences() {
		...
		echo '<p>';
		printf(
			__( 'This plugin requires at least %1$s and either %2$s, or %3$s, or %4$s', 'qts' ),
	...
	function init() {
		...
		if ( is_admin() ) {
			...
			if ( ! isset( $qts_options[ QTS_PREFIX . 'styles' ] ) || 'file' == $qts_options[ QTS_PREFIX . 'styles' ] ) {
				add_action( 'wp_enqueue_scripts', array( &$this, 'register_plugin_styles' ) );
			} elseif ( 'minified' == $qts_options[ QTS_PREFIX . 'styles' ] ) {
				add_action( 'wp_enqueue_scripts', array( &$this, 'register_plugin_styles_min' ) );
			} elseif ( 'inline' == $qts_options[ QTS_PREFIX . 'styles' ] ) {
	...
	public function get_slug( $id, $lang ) {
		$slug_array = get_post_meta( $id, '_qts_slug_' . $lang );
		return ! empty( $slug_array ) ? $slug_array[0] : '';
	}
	...
	*/
	...
	public function parse_url_args( $url ) {
		...
		if ( empty( $this->permalink_structure ) || 1 == $q_config['url_mode'] ) {
	...
	*/
	private function get_page_id_by_path( $page_path, $output = OBJECT, $post_type = 'page' ) {
		...
		foreach ( (array) $pages as $page ) {
			if ( $page->meta_value == $revparts[0] ) {
				...
				if ( 0 == $p->post_parent && count( $revparts ) == $count + 1 && $p->meta_value == $revparts[ $count ] ) {
					$foundid = $page->ID;
					break;
				}
			}
		...
		if ( $foundid ) {
			...
			$page_id = $wpdb->get_var(
				$wpdb->prepare(
					"SELECT ID FROM $wpdb->posts WHERE post_name = %s AND (post_type = %s OR post_type = 'attachment' )",
					$last_part,
					$post_type_sql
				)
			);
	 ...
	 * @param $leavename (bool ) Whether to keep the post name.
	 * @param $sample (bool ) Is it a sample permalink.
		...
		if ( 'edit.php' == $pagenow ) {
			...
			if ( ! empty( $terms ) ) {
				foreach ( $terms as $term ) {
					if ( $term instanceof WP_Term && isset( $meta[ $term->name ][ $lang ] ) ) {
						$term->name = $meta[ $term->name ][ $lang ];
					}
				}
			}
	...
	/**
	 * Hide automatically the wordpress slug box in edit posts page
	...
	public function validate_term_slug( $slug, $term, $lang ) {
		...
		$name = ( '' == $name_in_lang || 0 == strlen( $name_in_lang ) ) ? $term_name : $name_in_lang;
		$slug = trim( $slug );
	...
	public function unique_term_slug( $slug, $term, $lang ) {
		...
		$exists_slug = $wpdb->get_results( $query ); // WPCS: unprepared SQL OK.
	...
	public function save_term( $term_id, $tt_id, $taxonomy ) {
		...
		|| ( isset( $cur_screen ) && 'nav-menus' === $cur_screen->id )
		) {
			return $term_id;
		}
		...
		switch ( $type ) {
			...
			case 'both':
				...
				foreach ( $languages as $index => $lang ) :
					...
					if ( ( $num_languages - 1 ) == $index ) { $item_class[] = 'last-child'; }
